...
// Dashboard das conversas
app.get('/freshdesk/dashboard', async (req, res) => {
  try {
    const webhookService = FreshDeskWebhookService.getInstance();
    const dashboard = await webhookService.getConversationsDashboard();
    ...
    console.error('[FRESHDESK] Erro no dashboard:', error);
    res.status(500).json({
      success: false,
...
app.use('/api/v1', basicAuthMiddleware);
...
async function startServer() {
  ...
  try {
    ...
    // Inicializar LangGraph Agent (se estiver usando)
    try {
      const { LangGraphAgent } = require('./src/agent/langGraphAgent');
      const langGraphAgent = LangGraphAgent.getInstance();
      await langGraphAgent.initialize();
      console.log('✅ LangGraph Agent inicializado');
    } catch (error) {
      console.warn('⚠️ LangGraph Agent não disponível:', error instanceof Error ? error.message : 'Erro desconhecido');
    }
...
// Graceful shutdown
process.on('SIGINT', () => {
  console.log('\n📴 Recebido SIGINT. Parando servidor graciosamente...');
  process.exit(0);
});
