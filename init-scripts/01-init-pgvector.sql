...
RETURNS TRIGGER AS $$
BEGIN
    -- Se o conteúdo mudou, incrementa versão e atualiza hash
    IF OLD.content IS DISTINCT FROM NEW.content OR 
       ...
       OLD.problem_description IS DISTINCT FROM NEW.problem_description THEN
        ...
        -- Remove embeddings antigos para reprocessamento
...
CREATE TRIGGER check_support_solutions_content_changes
    BEFORE UPDATE ON support_solutions
    FOR EACH ROW
    EXECUTE FUNCTION check_content_changes();

-- Função para calcular hash no INSERT
